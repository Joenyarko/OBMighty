...
import { roleAPI, companyAPI } from '../services/api';
...
function Settings() {
    ...
    useEffect(() => {
        if (user) {
            ...
            // Initialize company data if available in user context (or fetch)
            if (user.company) {
                setCompanyData({
    ...
    // --- Company Customization Handlers ---
    const handleCompanyUpdate = async (e) => {
        e.preventDefault();
        setCompanyLoading(true);
        try {
            const response = await companyAPI.updateSettings(companyData);

            // Update context/local storage user company data if needed
            // Assuming setUser updates the user object which contains company
            const updatedUser = { ...user, company: response.data.company };
            setUser(updatedUser);

            // Update local storage
            const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
            localStorage.setItem('user', JSON.stringify({ ...storedUser, company: response.data.company }));

            // Live update custom properties if primary color changed
            if (companyData.primary_color) {
                document.documentElement.style.setProperty('--primary-color', companyData.primary_color);
                document.documentElement.style.setProperty('--yellow', companyData.primary_color);
            }
                ...
                {isCEO && (
                    <button
                        ...
                        style={{
                            ...
                            color: activeTab === 'company' ? 'var(--primary-color)' : 'var(--text-secondary)',
                            cursor: 'pointer',
                ...
                <div style={{ maxWidth: '600px' }}>
                    <div className="card" style={{ background: 'var(--card-bg)', padding: '32px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                        <form onSubmit={handleCompanyUpdate}>
                            <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '24px', color: 'var(--text-primary)' }}>Company Customization</h3>
                            ...
                            <div className="form-group" style={{ marginBottom: '20px' }}>
                                <label style={{ display: 'block', marginBottom: '8px', color: 'var(--text-secondary)', fontSize: '14px' }}>
                                    Card Prefix <span style={{ fontSize: '0.8em', color: 'var(--text-secondary)' }}>(e.g. OBM, ABC)</span>
                                </label>
                                <input
                                    type="text"
                                    value={companyData.card_prefix}
                                    onChange={e => setCompanyData({ ...companyData, card_prefix: e.target.value.toUpperCase() })}
                                    maxLength={5}
                                    placeholder="NEZ"
                                    style={{
                                        width: '100%',
                                ...
                                <p style={{ fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px' }}>
                                    This prefix will be used for all new cards generated (e.g., {companyData.card_prefix || 'NEZ'}-001).
                                </p>
                            </div>
